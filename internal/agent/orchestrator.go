...
func (a *Agent) runOrchestrated(ctx context.Context, userInput string) error {
	...
	msg, meta, err := a.Client.StreamChat(ctx, model, synth, nil, func(tok string) {
...
%s`, maxTasks, userInput)
	...
	msg, meta, err := a.Client.StreamChat(ctx, model, msgs, nil, nil)
	if err != nil {
		return nil, meta, err
	}
...
`, idx+1, len(tasks), runtime.GOOS, cwd, original)
	...
	for round := 0; round < maxSubagentRounds; round++ {
		msg, meta, err := a.Client.StreamChat(ctx, model, msgs, tools.Definitions(), func(tok string) {
			fmt.Fprint(a.Out, tok)
		})
		if err != nil {
			fmt.Fprintln(a.Out)

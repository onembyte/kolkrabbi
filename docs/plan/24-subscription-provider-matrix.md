...
| GitHub | Copilot Free, Pro, Pro+, Business, Enterprise | User-installed Copilot CLI handover: `copilot login`, then `copilot -p … -s --output-format json` (JSONL events, `--resume`, usage per call), every tool allowed only under kolk's full-auto; GitHub Models, the API path, was retired 2026-07-30 | **[x] shipped 2026-09-06** — verified live on the owner's Free plan (CLI 1.0.82): `auto` is the only model Free accepts, no effort on `auto`, one premium request per turn, a denied tool is the turn's error. Paid plans carry the Free row's shape as seeds until a login on one says more. ToS §J (2026-04-27) has no clause restricting Copilot to GitHub clients; inputs may train unless opted out |

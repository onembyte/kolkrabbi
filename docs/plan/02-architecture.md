...
| **Secrets** | never a config key; keychain or `KOLK_OPENROUTER_KEY`; `secret.Redact` runs over sessions, stats and logs | — |
| **Commands** | one word, lowercase, ≤ 6 letters, no synonyms; grammar `kolk <verb> [subverb] [args]` | ship: `model effort mode config models sessions stats dash saga serve login doctor update help version` |

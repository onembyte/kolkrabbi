...
| `claude-permission-denied.ndjson` | kolk's own argv: `claude -p --verbose --output-format stream-json --input-format stream-json --safe-mode --setting-sources "" --tools Bash,… --permission-mode acceptEdits --model fable --effort medium`, one stream-json user turn asking for `ls -la && go version` | 19 frames: two `system/permission_denied` whose `message` is a **plain string** (not an object), each followed by the `user`(`tool_result`, `is_error`) that carries the same reason; also `system/thinking_tokens`. Captured 2026-09-02 with **Claude Code 2.1.258**, model `fable` |

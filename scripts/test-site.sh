...
contains index.html 'Installer ships with v1.3.3' "current installer release status is missing"
contains index.html '"softwareVersion": "1.3.3"' "structured data version drifted from the installer release"
...
contains index.html 'class="nav-button" href="/capabilities"' "landing page has no capabilities navbar button"
...
contains capabilities.html 'class="nav-button" href="/capabilities" aria-current="page"' "capabilities navbar state is missing"
...
contains local-ai-coding-agent.html 'It never goes looking on your network' "the local guide must say kolk probes only what it is told to"
contains local-ai-coding-agent.html '/localia add &lt;name&gt; &lt;host:port&gt;' "the local guide does not show how to add a machine (plan 37)"
contains local-ai-coding-agent.html 'A machine that is not this one' "the local guide does not cover a LAN or Tailscale endpoint"
not_contains local-ai-coding-agent.html 'It only ever probes 127.0.0.1:11434' "the local guide still claims kolk probes one address only"
